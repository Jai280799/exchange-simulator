...
def test_trade_emitted_on_volume_increase_only():
    rows = [
        _row("90000000", "100", last_px="100.5", size="100"),  # opening trade, delta from 0
        _row("90000050", "100"),                               # no volume change -> no trade
        _row("90000100", "130", last_px="100.6", size="30"),   # +30 traded
    ]
    msgs = list(iter_messages(rows, instrument_id="2603"))
    trades = [m for m in msgs if isinstance(m, MarketTradePrint)]
    assert len(trades) == 2
    assert trades[0].quantity == 100 and trades[0].price == Decimal("100.5")
    assert trades[1].quantity == 30 and trades[1].cumulative_volume == 130


def test_sequence_is_unique_and_book_precedes_its_trade():
    rows = [_row("90000000", "50", last_px="100.5", size="50")]
    msgs = list(iter_messages(rows, instrument_id="2603"))
    assert [m.sequence for m in msgs] == [0, 1]
    assert isinstance(msgs[0], MarketDataSnapshot)
    assert isinstance(msgs[1], MarketTradePrint)
